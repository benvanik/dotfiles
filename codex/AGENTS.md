...
repo-local `AGENTS.md`, `AGENTS.override.md`, or fallback project doc adds more
specific rules.
...
- If a project uses beads/br/bd or another issue log, treat issue text as intent
  snapshots, not source-of-truth facts. Reproduce and verify against the
  filesystem before acting on claims in tickets.
- Claim work when you start and update/close it according to the project-local
  workflow, but do not let bookkeeping replace technical verification.
