"""Private generated receipts for exact service installations."""
...
from .instances import InstanceStore, OPERATION_ID_PATTERN
from .remote import SshEndpoint
...
INSTALLATION_SCHEMA = "runpod.inference-service-installation.v1"
INSTALLATION_IDENTITY_SCHEMA = "runpod.inference-service-installation-identity.v1"
SERVICE_CONTRACT_SCHEMA = "runpod.inference-service-request.v1"
...
class ServiceInstallationStore:
    """Crash-safe generated installation bindings beneath RUNPOD_HOME."""
    ...
    def _document(
        ...
        *,
        ...
        if materialization.root != expected_root:
            _fail(
                "installed materialization is outside this RUNPOD_HOME",
                code="invalid_service_installation",
            )
    ...
    def publish(
        ...
        *,
        ...
        if (
            not isinstance(instances, InstanceStore)
            or instances.state is not self.state
        ):
            _fail(
                "installation publication requires this state store's "
                "instance lease guard",
                code="invalid_service_installation",
            )
        ...
        with instances.locked_active_lease(
            ...
            with self.state.locked(_lock_name(endpoint.instance_name, service_id)):
                existing = self.state.read(INSTALLATION_NAMESPACE, record_name)
                if existing != desired:
                    # Remote success authorizes replacement. Requiring the previous
                    # receipt's local materialization here would make a fresh
                    # install unable to recover from lost generated state.
                    self.state.write(
                        INSTALLATION_NAMESPACE,
                        record_name,
                        desired,
    ...
    def load_selector(
        ...
        """Resolve one exact generated materialization beneath RUNPOD_HOME."""
        ...
        if SHA256_PATTERN.fullmatch(value):
            ...
            if SHA256_PATTERN.fullmatch(
                ...
                _fail(
                    ...
                    "identity or its RUNPOD_HOME path",
