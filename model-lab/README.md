...
The example hard deadline covers the profile's 12-hour Pi runtime ceiling plus
its 30-minute idle grace; it is a fail-safe, not the normal shutdown path.
...
1. validate the profile and permanent service binding;
...
Images can be attached through Pi’s normal file-reference syntax:

```text
@/project/reference.png Describe this image.
```

The profile and endpoint must both advertise the `image` modality. Text-only
profiles fail before launch instead of silently dropping the image.
...
Legacy profile migration is provider-free and source-preserving:

```sh
model-lab migrate /absolute/legacy/profile \
  --service qwen36-heretic-nvfp4 \
  --target-profile-id chat
```

It holds the legacy production materialization and run locks, verifies exact
...
retirement independently and reports an aggregate failure when any owned
cleanup remains.
...
billable bound if the supervisor and machine both disappear.
