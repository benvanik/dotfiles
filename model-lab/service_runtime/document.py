"""Strict parser for one generated model-service deployment manifest."""
...
def _normalized_service(value: Any) -> dict[str, Any]:
    ...
    _choice(
        model["weight_format"],
        label="definition.service.model.weight_format",
        choices=frozenset({"native", "bf16", "fp8", "int8", "q8"}),
    )
    ...
    _integer(
        ...
        label="definition.service.endpoint.max_output_tokens",
        minimum=1,
        maximum=2**24,
    )
    ...
    resources = _exact_fields(
        service["resources"],
        label="definition.service.resources",
        fields=frozenset(
            {
                "gpu_count",
                "gpu_memory_gib",
    ...
    _integer(
        resources["gpu_count"],
        label="resources GPU count",
        minimum=1,
        maximum=16,
    )
    _integer(
        resources["gpu_memory_gib"],
        label="resources GPU memory",
        minimum=1,
        maximum=4096,
    )
    _integer(
        resources["cpu_count"],
        label="resources CPU count",
        minimum=1,
        maximum=1024,
    )
    _integer(
        resources["memory_gib"],
        label="resources memory",
        minimum=1,
        maximum=16384,
    )
    _integer(
        resources["ephemeral_disk_gib"],
        label="resources ephemeral disk",
        minimum=1,
        maximum=65536,
    )
    _choice(
        resources["claim_mode"],
        label="resources claim mode",
        choices=frozenset(
...
def _runtime_selection(value: Any, *, expected_runtime_id: str) -> dict[str, Any]:
    ...
    if runtime["schema_version"] != "model-lab.runtime-selection.v1":
...
def parse_deployment_manifest(payload: bytes) -> DeploymentManifest:
    ...
    if implementation["remote_root"] != str(
        ...
        expected_implementation_root / "bin" / "model-lab-service-runtime"
    ):
