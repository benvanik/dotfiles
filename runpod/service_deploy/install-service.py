#!/usr/bin/python3.12
"""Install one exact inference-service materialization without replacing state."""
...
IMPLEMENTATION_ROOT = SESSION_ROOT / "control" / "inference-service-runtime"
...
def prepare(identity: str, transfer_id: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": "runpod.inference-service-install-operation.v1",
        "action": "prepare",
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": ("runpod.inference-service-install-error.v1"),
