...
def closure_for(definition: InferenceServiceDefinition) -> HuggingFaceClosure:
...
class ServiceMaterializationTest(unittest.TestCase):
    ...
    def test_complete_materialization_rejects_tamper_and_extra_files(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(RunpodLocalError) as tampered:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(RunpodLocalError) as unexpected:
                load_service_materialization(materialized.root)
            self.assertEqual(
